...
/**
 * Driver SOS (spec 04) — human safety emergencies. Press & HOLD 2s →
 * real POST /api/alerts (type sos) → red banner on the RTC dashboard.
 * Distinct from Breakdown (mechanical).
 */
export default function DriverSOS() {
  ...
  const fireSOS = async () => {
    ...
    try {
      ...
      await tripService.sendSOS({
        trip_id: activeTripId ?? undefined,
        license_plate: bus?.regNo,
        route_number: route?.number,
        driver_phone: cleanPhone,
        description: 'DRIVER_EMERGENCY',
        latitude: lat,
        longitude: lng,
      });
      setSent(true);
      Alert.alert('SOS Sent', 'Depot control room notified with your live location.');
    } catch (err: any) {
      console.error('SOS error:', err);
      Alert.alert('Failed', err?.message || 'Server unreachable — call 112 directly if this is an emergency.');
    } finally {
      setSending(false);
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>
          Confirm to notify emergency services and depot contacts.
        </Text>
            ...
            >
              {sending ? (
                ...
                <>
                  ...
                  <Text style={styles.sosHint}>{sent ? 'ALERT SENT' : 'PRESS & HOLD'}</Text>
                </>
              )}
            </Pressable>
          </Animated.View>
        ...
        {[
          { initial: 'DC', name: 'Depot Control', tag: 'Primary Contact', color: BRAND.primary },
          { initial: 'EM', name: 'Emergency (112)', tag: '+91-112', color: BRAND.danger },
        ].map((c) => (
